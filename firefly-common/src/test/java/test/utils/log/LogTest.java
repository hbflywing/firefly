...
public class LogTest {
	...
	private static final Log log5 = LogFactory.getInstance()
			.getLog("test-WARN");
	private static final Log log6 = LogFactory.getInstance()
			.getLog("test-INFO");
	...
	public void test() {
		...
		try {
			...
			// test debug
			FileUtils.read(getFile(log3), new LineReaderHandler(){
			...
			// test info
			FileUtils.read(getFile(log6), new LineReaderHandler(){
			...
			// test warn
			FileUtils.read(getFile(log5), new LineReaderHandler(){
			...
			// test error
			FileUtils.read(getFile(log4), new LineReaderHandler(){
